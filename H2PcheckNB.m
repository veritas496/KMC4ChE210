function [NZE,HdiffusionSP,CH4DSP,H2PSP,CH3DSP,CH2DSP,CH3PSP,CHDSP,CH2PSP,CdiffusionSP,CHPSP,C3diffusionSP,CH4PSP]=H2PcheckNB(NZE,Conf,W,L,H,CH3,CH2,CH,C,C3,...
...
if (H1NBup(2)<=L)&sum((H1NBup~=H2pickedup).^2)~=0
    ...
    switch (Checkspecies)
        case(0)
            ...
            HdiffusionSP(H1pickedup(1),H1pickedup(2))=HdiffusionSP(H1pickedup(1),H1pickedup(2))-1;%removeevent(HdiffusionSP,x,'HdiffusionSP');
            if HdiffusionSP(H1pickedup(1),H1pickedup(2))==0
                NZE(1)=NZE(1)-1;
            end
        case(H)
...
if (H1NBdown(2)>=1)&sum((H1NBdown~=H2pickedup).^2)~=0
...
if (H1NBleft(1)>=1)&sum((H1NBleft~=H2pickedup).^2)~=0
...
if (H1NBright(1)<=W)&sum((H1NBright~=H2pickedup).^2)~=0
...
if (H2NBup(2)<=L)&sum((H2NBup~=H1pickedup).^2)~=0
    ...
    switch (Checkspecies)
        case(0)
            ...
            HdiffusionSP(H2pickedup(1),H2pickedup(2))=HdiffusionSP(H2pickedup(1),H2pickedup(2))-1;%removeevent(HdiffusionSP,x,'HdiffusionSP');
            if HdiffusionSP(H2pickedup(1),H2pickedup(2))==0
                NZE(1)=NZE(1)-1;
            end
        case(H)
...
if (H2NBdown(2)>=1)&sum((H2NBdown~=H1pickedup).^2)~=0
...
if (H2NBleft(1)>=1)&sum((H2NBleft~=H1pickedup).^2)~=0
...
if (H2NBright(1)<=W)&sum((H2NBright~=H1pickedup).^2)~=0
