...
while coverage<90
    ...
    [Ea]=energybarrier(coverage,t,CXFNum);
    k=A.*exp(-Ea./(R*T));
    rate=k.*NZE;
   ...
   if  rate(7)>0 % r7 is CH2 dissociation
            totalrate=sum(rate);
            rest=totalrate-rate(1);   % Do not account for H surface diffusion 
            numstep=rate(1)/rest;
            
            %pick up the event i
            
            if RN<=(sum(rate(3))/rest)
