function [NZE,CH2PSP,CH3PSP,CHDSP,HdiffusionSP,CH4DSP,H2PSP,CH3DSP,CH2DSP,CdiffusionSP,CHPSP,C3diffusionSP,CH4PSP]=CH2DcheckNB(NZE,Conf,W,L,H,CH2PSP,CH3PSP,CHDSP,...
    CH3,CH2,CH,C,C3,HdiffusionSP,CH4DSP,H2PSP,CH3DSP,CH2DSP,CdiffusionSP,CHPSP,C3diffusionSP,CH4PSP,...
    CHpickedup,CHNBup,CHNBdown,CHNBleft,CHNBright,Hpickedup,HNBup,HNBdown,HNBleft,HNBright)
%x is CHpickedup;y is one of its neighbour

if (CHNBup(2)<=L)&sum((CHNBup~=Hpickedup).^2)~=0
    Checkspecies=Conf(CHNBup(1),CHNBup(2)); %check CH neighbour
    
    switch (Checkspecies)
        case(0)
            CHDSP(CHpickedup(1),CHpickedup(2))=CHDSP(CHpickedup(1),CHpickedup(2))+1;%addevent(CHDSP,x,'CHDSP');
            if CHDSP(CHpickedup(1),CHpickedup(2))==1
                NZE(8)=NZE(8)+1;
            end
        case(H)
            CH2PSP(CHpickedup(1),CHpickedup(2))=CH2PSP(CHpickedup(1),CHpickedup(2))+1;%addevent(CH2PSP,x,'CH2PSP');

            CH3PSP(CHpickedup(1),CHpickedup(2))=CH3PSP(CHpickedup(1),CHpickedup(2))-1;%removeevent(CH3PSP,x,'CH3PSP');
            if CH3PSP(CHpickedup(1),CHpickedup(2))==0
                NZE(16)=NZE(16)-1;
            end
    end
end

if (CHNBdown(2)>=1)&sum((CHNBdown~=Hpickedup).^2)~=0
    Checkspecies=Conf(CHNBdown(1),CHNBdown(2)); %check CH neighbour
...
if (CHNBleft(1)>=1)&sum((CHNBleft~=Hpickedup).^2)~=0
    Checkspecies=Conf(CHNBleft(1),CHNBleft(2)); %check CH neighbour
...
if (CHNBright(1)<=W)&sum((CHNBright~=Hpickedup).^2)~=0
    Checkspecies=Conf(CHNBright(1),CHNBright(2)); %check CH neighbour
...
if (HNBup(2)<=L)&sum((HNBup~=CHpickedup).^2)~=0
...
if (HNBdown(2)>=1)&sum((HNBdown~=CHpickedup).^2)~=0
...
if (HNBleft(1)>=1)&sum((HNBleft~=CHpickedup).^2)~=0
...
if (HNBright(1)<=W)&sum((HNBright~=CHpickedup).^2)~=0
