function [NZE,CHPSP,C2FSP,C3FSP,CXFSP,CdiffusionSP,HdiffusionSP,CH4DSP,CH3DSP,CH2DSP,CHDSP,C3diffusionSP]=redefineC2FcheckNB(NZE,Conf,W,L,H,C,C2,CX,CHPSP,C2FSP,C3FSP,CXFSP,...
...
if C1NBup(2)<=L
    Checkspecies=Conf(C1NBup(1),C1NBup(2)); %check C2 (the second C) neighbour
    switch (Checkspecies)
        %case(0)
        ...
        case(C)
            ...
            C2FSP(C1pickedup(1),C1pickedup(2))=C2FSP(C1pickedup(1),C1pickedup(2))-1;%removeevent(C2FSP,x,'C2FSP');
            if C2FSP(C1pickedup(1),C1pickedup(2))==0
                NZE(9)=NZE(9)-1;
            end
...
if C1NBdown(2)>=1
    Checkspecies=Conf(C1NBdown(1),C1NBdown(2)); %check C2 (the second C) neighbour
    switch (Checkspecies)
        %case(0)
...
if C1NBleft(1)>=1
    Checkspecies=Conf(C1NBleft(1),C1NBleft(2)); %check C2 (the second C) neighbour
    switch (Checkspecies)
        %case(0)
...
if C1NBright(1)<=W
    Checkspecies=Conf(C1NBright(1),C1NBright(2)); %check C2 (the second C) neighbour
    switch (Checkspecies)
        %case(0)
...
if (C2NBup(2)<=L)
    ...
    Checkspecies=Conf(C2NBup(1),C2NBup(2)); %check C1 neighbour
    ...
    switch (Checkspecies)
        ...
        case(C)
            ...
            C2FSP(C2pickedup(1),C2pickedup(2))=C2FSP(C2pickedup(1),C2pickedup(2))-1;%removeevent(C2FSP,y,'C2FSP');
            if C2FSP(C2pickedup(1),C2pickedup(2))==0
                NZE(9)=NZE(9)-1;
            end
...
if (C2NBdown(2)>=1)
    ...
    Checkspecies=Conf(C2NBdown(1),C2NBdown(2)); %check C1 neighbour
...
if (C2NBleft(1)>=1)
    ...
    Checkspecies=Conf(C2NBleft(1),C2NBleft(2)); %check C1 neighbour
...
if (C2NBright(1)<=W)
    ...
    Checkspecies=Conf(C2NBright(1),C2NBright(2)); %check C1 neighbour
